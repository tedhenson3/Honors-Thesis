...
season.avgs = aau.data.clean %>% group_by(Player, Season) %>%
                summarise(aau.mp = mean(MP),
                          ...
                          aau.pf = mean(PF),
                          aau.pts = mean(PTS),
                          aau.ft.made  = mean(as.numeric(word(FT, start = 1, end = 1,
                                                  sep = '-'))),
                          aau.fg.made  = mean(as.numeric(word(FG, start = 1, end = 1,
                                                             sep = '-'))),
                          aau.threep.made  = mean(as.numeric(word(`3P`, start = 1, end = 1,
                                                             sep = '-'))))


season.totals= aau.data.clean %>% group_by(Player, Season) %>%
  summarise(aau.sum.mp = sum(MP),
            aau.sum.reb = sum(REB),
            aau.sum.blk  = sum(BLK),
            aau.sum.stl  = sum(STL),
            aau.sum.to = sum(TO),
            aau.sum.pf = sum(PF),
            aau.sum.pts = sum(PTS),
            aau.sum.ft.made  = sum(as.numeric(word(FT, start = 1, end = 1,
                                                sep = '-'))),
            aau.sum.fg.made  = sum(as.numeric(word(FG, start = 1, end = 1,
                                                sep = '-'))),
            aau.sum.threep.made  = sum(as.numeric(word(`3P`, start = 1, end = 1,
                                                    sep = '-'))))
