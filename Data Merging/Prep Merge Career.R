...
colnames(prep.total)[which(grepl('.prep', colnames(prep.total)) == F)] = 
  paste(colnames(prep.total)[which(grepl('.prep', colnames(prep.total)) == F)],
        'prep', sep = '.')

colnames(prep.total) = gsub('Name.prep', 'Name', colnames(prep.total))

colnames(prep.total) = gsub('Season.prep', 'Season', colnames(prep.total))

colnames(prep.total) = gsub('player.id.prep', 'player.id', colnames(prep.total))


write.csv(prep.total, file = 'Prep.Career.csv', row.names = F)
