...
# Methods
  All models were trained through leave one out cross validation. Each player's predicted win shares was outputted from a function fitted on all other players. In order to speed up these computations, hyperparameters for some of the different methods were trained through 10 fold cross validation. These values were chosen based on the parameters yielding the smallest out of sample error. For example, in lasso regression, all models used the same optimal lambda value during the fitting process.
  
  In addition to predicting raw win shares, a seperate set of predictions were also generated by predicting a players offensive and defensive win shares seperately. The final predictions were a simple sum of these two quantities. Predictions were also generated using a weighted sum, but these were less accurate. Below are the tables of out of sample RMSE for the different input combinations.

# Results
## Leave One Out Win Share RMSEs
```{r, echo = F, eval = T, include=F}
rmse.table = read_csv("~/Honors Thesis/Predictions/All RMSEs (raw ws 5 folds).csv")
#colnames(rmse.table)[1] = 'Model'
rmse.table[,2:ncol(rmse.table)] = round(rmse.table[,2:ncol(rmse.table)], 2)
colnames(rmse.table)[1] = 'Method'
```
`r pander(rmse.table)`
## Leave One Out Offensive + Defensive Win Share RMSEs
```{r, echo = F, eval = T, include=F}
rmse.table = read_csv("~/Honors Thesis/Predictions/All RMSEs (ows+dws).csv")
#colnames(rmse.table)[1] = 'Model'
rmse.table[,2:ncol(rmse.table)] = round(rmse.table[,2:ncol(rmse.table)], 2)
colnames(rmse.table)[1] = 'Method'
...
# Important Variables
## Full Model Ridge Regression Coefficients
```{r, echo = F, eval = T, include=F}
# load(paste("~/Honors Thesis/Model Environments/", "ridge", ' ', 'full', ".RData", sep = ""))
# print(coefficients(train.fit))
