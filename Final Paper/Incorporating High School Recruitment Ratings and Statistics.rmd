...
\item $f_{AAU},\:f_{Prep},\:f_{ESPN},\:and\:f_{FULL}\:are\:functions\:that\:output\:predicted\:win\:shares\:based\:each\:data\:source,$
...
# Methods
  ...
  In addition to predicted raw win shares, a seperate set of predictions were also generated by predicting a players offensive and defensive win shares seperately. The final predictions were a weighted sum of these two quantities. The linear weights were calculated through leave one out cross validation. A weighted sum was used as this significantly decreased out of sample accuracy compared to adding the two together.

# Results
## Leave One Out Win Share RMSEs
```{r, echo = F, eval = T, include=F}
rmse.table = read_csv("~/Honors Thesis/Predictions/All RMSEs (complete players more covariates).csv")
...
```{r, echo = F, eval = T, include=F}
rmse.table = read_csv("~/Honors Thesis/Predictions/All RMSEs (complete players ows+dws regressed).csv")
#colnames(rmse.table)[1] = 'Model'
rmse.table[,2:ncol(rmse.table)] = round(rmse.table[,2:ncol(rmse.table)], 2)
```
`r pander(rmse.table)`
