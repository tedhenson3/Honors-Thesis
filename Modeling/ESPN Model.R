...
knn.impute <- preProcess(train[,6], method=c("knnImpute"))
# # 
train[,6] <- predict(knn.impute, newdata = train[,6])



ws.formula = as.formula(
  'ws  ~
espn.rating + poly(espn.rating, degree = 3)
...
models.espn <- caretList(form = ws.formula, data=train,  trControl=fitControl, methodList=algorithmList) 
results <- resamples(models.espn)
summary(results)
...
espn.stack.glm <- caretStack(models.espn, 
                        method = "glm", 
...
test[,6] <- predict(knn.impute, newdata = test[,6])




#predict WAR values using your model
#bart =  predict(models.espn$bartMachine, test)


bridge.espn = predict(models.espn$bridge, test)
rf.espn = predict(models.espn$rf, test)
brnn.espn = predict(models.espn$brnn, test)
stacked.espn <- predict(espn.stack.glm, test)
blasso.espn <- predict(models.espn$blasso, test)


#compute the residuals
espn.error.stacked <- test$ws - stacked.espn
espn.error.bridge <- test$ws - bridge.espn
espn.error.rf <- test$ws - rf.espn
...
espn.matrix[1,1] = 'Stacked Model'
espn.matrix[2,1] = 'Bayesian Ridge'
espn.matrix[3,1] = 'Random Forest'
...
library(monomvn)
# bl = models.prep$blasso$finalModel
...
ggplot() + 
  ...
  geom_point(aes(x = stacked.espn, y = espn.error.stacked),
             colour = 'skyblue', size = 2.5) + 
