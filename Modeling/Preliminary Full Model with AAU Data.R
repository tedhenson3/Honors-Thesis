...
data = left_join(x = data, y = aau, by = c('player.id', 'Season.x'))
...
data = data %>% dplyr::select(Name.x, ws, ws_per_40,
                              ...
                              FTM,
                              `FT%`,
                              aau.mp, aau.reb,  aau.blk, aau.stl,
                              aau.to,
                              aau.pf,
                              aau.pts,
                              aau.ft.made,
                              aau.fg.made,
                              aau.threep.made,
                              aau.sum.mp, aau.sum.reb,  aau.sum.blk, aau.sum.stl,
                              aau.sum.to,
                              aau.sum.pf,
                              aau.sum.pts,
                              aau.sum.ft.made,
...
knn.impute <- preProcess(train[,7:ncol(train)], method=c("knnImpute"))
# 
train[,7:ncol(train)] <- predict(knn.impute, newdata = train[,7:ncol(train)])
...
test[,7:ncol(test)] <- predict(knn.impute, newdata = test[,7:ncol(test)])
...
ggplot() + 
  ...
  geom_point(aes(x = stacked.full, y = full.error.stacked)
             , colour = 'skyblue',
             size = 2.5) + 
