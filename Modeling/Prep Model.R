...
#ggplot(train) + geom_point(aes(y = ws, x = PTS)) + xlab('Points Per Game') + ylab('Win Shares')
...
models.prep <- caretList(form = ws.formula, data=train,  trControl=fitControl, methodList=algorithmList) 
results <- resamples(models.prep)
summary(results)
...
prep.stack.glm <- caretStack(models.prep, 
                        method = "glm", 
...
#bart =  predict(models.prep$bartMachine, test)


bridge.prep = predict(models.prep$bridge, test)
rf.prep = predict(models.prep$rf, test)
brnn.prep = predict(models.prep$brnn, test)
stacked.prep =  predict(prep.stack.glm, test)
blasso.prep =  predict(models.prep$blasso, test)


#compute the residuals
prep.error.stacked <- test$ws - stacked.prep
prep.error.bridge <- test$ws - bridge.prep
prep.error.rf <- test$ws - rf.prep
...
prep.inputs.matrix[1,1] = 'Stacked Model'
prep.inputs.matrix[2,1] = 'Bayesian Ridge'
prep.inputs.matrix[3,1] = 'Random Forest'
...
ggplot() + 
  ...
  geom_point(aes(x = bridge.prep, y = prep.error.bridge), colour = 'skyblue',
             size = 2.5) + 
